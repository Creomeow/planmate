...
Keep responses concise but informative. Focus on being helpful and practical.`;
      ...
      // Call Cohere API (using a free endpoint)
      const response = await fetch('https://api.cohere.ai/v1/chat', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer demo-key', // This will use fallback
        },
        body: JSON.stringify({
          message: userMessage,
          model: 'command',
      ...
      const assistantResponse = data.text || 'Sorry, I couldn\'t process that request.';
