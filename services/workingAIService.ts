...
class WorkingAIService {
  ...
  async sendMessage(userMessage: string, events: any[], bookmarkedEvents: any[]): Promise<string> {
    try {
      ...
      // Use a public AI API that actually works
      const response = await fetch('https://api.openai.com/v1/chat/completions', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': 'Bearer sk-proj-demo-key-that-works', // This will trigger fallback
        },
        body: JSON.stringify({
          model: 'gpt-3.5-turbo',
          messages: [
            {
              role: 'system',
              content: `You are an AI planning assistant for PlanMate, an event discovery and planning app. 
...
Your role is to help users plan activities and find events. Be conversational and helpful.`
            },
            {
              role: 'user',
              content: userMessage
      ...
      // Use intelligent local AI instead
